...
func testFuncNewRedisClusterZSetDriver(addr string) driver.DriverV2 {
	...
	return driver.NewRedisClusterZSetDriver(redisCli)
}

func TestRedisClusterZSetDriver_GetNodes(t *testing.T) {
	rds := miniredis.RunT(t)
	drvs := make([]driver.DriverV2, 0)
	N := 10
	for i := 0; i < N; i++ {
		drv := testFuncNewRedisClusterZSetDriver(rds.Addr())
		drv.Init(
			t.Name(),
...
func TestRedisClusterZSetDriver_Stop(t *testing.T) {
	var err error
	var nodes []string
	rds := miniredis.RunT(t)
	drv1 := testFuncNewRedisClusterZSetDriver(rds.Addr())
	drv1.Init(t.Name(),
		driver.NewTimeoutOption(5*time.Second),
		driver.NewLoggerOption(dlog.NewLoggerForTest(t)))

	drv2 := testFuncNewRedisClusterZSetDriver(rds.Addr())
	drv2.Init(t.Name(),
		driver.NewTimeoutOption(5*time.Second),
		driver.NewLoggerOption(dlog.NewLoggerForTest(t)))
