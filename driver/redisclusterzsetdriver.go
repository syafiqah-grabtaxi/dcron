...
type RedisClusterZSetDriver struct {
...
func newRedisClusterZSetDriver(redisClient *redis.ClusterClient) *RedisClusterZSetDriver {
	rd := &RedisClusterZSetDriver{
		c: redisClient,
...
func (rd *RedisClusterZSetDriver) Init(serviceName string, opts ...Option) {
...
func (rd *RedisClusterZSetDriver) NodeID() string {
	return rd.nodeID
}

func (rd *RedisClusterZSetDriver) GetNodes(ctx context.Context) (nodes []string, err error) {
	rd.Lock()
	defer rd.Unlock()
...
func (rd *RedisClusterZSetDriver) Start(ctx context.Context) (err error) {
	rd.Lock()
	defer rd.Unlock()
	if rd.started {
...
func (rd *RedisClusterZSetDriver) Stop(ctx context.Context) (err error) {
	rd.Lock()
	defer rd.Unlock()
	rd.runtimeCancel()
	rd.started = false
...
func (rd *RedisClusterZSetDriver) withOption(opt Option) (err error) {
	switch opt.Type() {
...
func (rd *RedisClusterZSetDriver) heartBeat() {
...
func (rd *RedisClusterZSetDriver) registerServiceNode() error {
